        ...
        if self._stream_ts is not None:
            if self._use_slack_stream:
                ...
                if self._stream_delta_dropped:
                    ...
                    # duplicated). Delivery-confirming by design: the update
                    # and any overflow posts are UNGUARDED — this is the
                    # answer-carrying call, and a raise here means the content
                    # is genuinely missing, so the transport catch-all
                    # recording a failure (and its partial-progress rescue) is
                    # the honest outcome.
                    seg = _display_safe(self._segment_text)
                    seg_chunks = await self._split_for_slack(seg or _NO_RESPONSE)
                    await self.slack.update_message(self.channel, self._stream_ts, seg_chunks[0])
            ...
            # Deliberately UNGUARDED: this is the answer-carrying call, not
            # decoration. If it raises too, nothing was delivered at all — the
            # transport catch-all recording a failure is the honest outcome,
            # and swallowing here would convert total delivery failure into
            # recorded success.
