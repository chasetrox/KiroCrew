...
async def api_dashboard_config(request: web.Request) -> web.Response:
    ...
    if request.method == "PUT":
        ...
        if body_err is not None:
            ...
            return body_err
        assert body is not None  # read_bounded_json returns (dict, None) on success
        _allowed = {"restore_sessions", "restore_window_minutes", "merge_queued_messages", "default_memory_mode", "widget_density", "use_builtin_browser", "verbosity", "quick_send", "session_grid", "replay_from_acp", "tail_fork_enabled", "link_previews", "mcp_app_panel", "auto_open_git_panel", "folder_suggestions_enabled", "session_card_source_links"}
