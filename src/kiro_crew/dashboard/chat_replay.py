...
def _is_prompt_row(row: dict[str, Any]) -> bool:
    ...
    if role in _PROMPT_ROLES:
        # A steer is delivered INSIDE the running turn on the kiro backend, so
        # it does not head a group up front; the kas backend's steer-as-its-own-
        # prompt is promoted by _split_steer_heads when the replay shows
        # a turn for it.
...
#: The kas (v3) backend delivers a steer as its OWN prompt, wrapped in this tag
#: pair; kiro (v1/v2) injects it into the running turn and no such prompt
#: exists. Only a prompt carrying the wrapper is ever compared with a steer row.
_STEER_PROMPT_WRAP = re.compile(r"</?user_message>")


def _steer_prompt_text(turn: dict[str, Any]) -> str:
    """The steer text a kas steer-as-prompt turn carries, or ``""`` for any other turn."""
    text = clean_user_text(turn.get("prompt_text") or "")
    if not _STEER_PROMPT_WRAP.search(text):
        return ""
    return _norm(_fold_dashes(_STEER_PROMPT_WRAP.sub("", text)))


def _is_steer_for(row: dict[str, Any], needle: str) -> bool:
    if row.get("role") != "user" or not _meta_of(row).get("steer"):
        return False
    text = _norm(_fold_dashes(_row_text(row)))
    if not text:
        return False
    return text == needle or (len(text) >= _CONTAINMENT_MIN_CHARS and text[:120] in needle)


def _split_steer_heads(
    groups: list[tuple[dict[str, Any] | None, list[dict[str, Any]]]],
    turns: list[dict[str, Any]],
) -> None:
    """Split JSONL groups at the steer rows that replayed as prompts of their own.

    A steer row never starts a group up front (:func:`_is_prompt_row`): on the
    kiro backend the steer lands INSIDE the running turn, so the answer and tool
    rows that follow it in the JSONL still belong to the turn the replay opened
    for the original prompt, and splitting there would render them twice. The
    kas backend instead delivers a steer as its own ``<user_message>`` prompt,
    so the replay opens a turn for it -- and with the JSONL steer parked in the
    previous group, that turn found no prompt, rendered the wire text as a grey
    user bubble, and the group's later rows (the steered answer) came out twice.

    So the split is driven by the replay: only a steer row that some replayed
    turn names (wrapper present, exact or containment) becomes a group head,
    taking the rows after it. It has to happen BEFORE the overlay walk, because
    the group holding the steer is the one the PREVIOUS turn overlays. The
    kiro backend never produces a wrapped prompt, so nothing changes there.
    """
    needles = [n for n in (_steer_prompt_text(t) for t in turns) if n]
    if not needles:
        return
    k = 0
    while k < len(groups):
        ...
        for i, row in enumerate(rest):
            if any(_is_steer_for(row, n) for n in needles):
                groups[k] = (prow, rest[:i])
                groups.insert(k + 1, (row, rest[i + 1 :]))
                break
        k += 1


def _match_steer_head(
    groups: list[tuple[dict[str, Any] | None, list[dict[str, Any]]]],
    start: int,
    turn: dict[str, Any],
) -> int:
    """Index of the first group from ``start`` headed by the steer row ``turn`` replays, else -1."""
    needle = _steer_prompt_text(turn)
    if not needle:
        return -1
    for k in range(start, len(groups)):
        prow = groups[k][0]
        if prow is not None and _is_steer_for(prow, needle):
            return k
    return -1
...
def merge_replay_transcript(
    ...
    # kas steers replayed as prompts of their own: split their groups before
    # anything is overlaid (the row sits in the group the PREVIOUS turn claims).
    _split_steer_heads(groups, turns)
    ...
    for turn in turns:
        ...
        if j == -1:
            # The wrapped kas steer prompt never equals its bare JSONL steer row,
            # so match it against the heads _split_steer_heads promoted.
            j = _match_steer_head(groups, cursor, turn)
        if j == _AMBIGUOUS:
