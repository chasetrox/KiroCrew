...
#: Tool calls the ENGINE issues for itself around a session -- never a step of
#: the agent's answer, never shown by the live path (they happen outside any
#: ``session/prompt``, where the dispatch loop counts-and-drops them). A
#: ``session/load`` replay carries them anyway, positioned wherever the engine
#: ran them: before the first prompt (skipped below because no turn is open)
#: but ALSO after the last answer, where the pre-turn rule cannot catch them and
#: they rendered as trailing ``fetch_cloud_config`` tool cards on the kas/v3
#: backend. Filtered by name, the one identity such a call carries on every
#: engine version (``_meta.kiro.toolName`` on v3, the raw title on v1/v2).
ENGINE_INTERNAL_TOOLS: frozenset[str] = frozenset({"fetch_cloud_config"})
...
def fold_replay_updates(
    ...
    *,
    ...
    belong to no turn and are skipped; engine-internal tool calls
    (:data:`ENGINE_INTERNAL_TOOLS`) are skipped wherever they fall, with their
    updates and results; other update kinds (plan, mode/config, session_info)
    are not transcript rows on the live path either.
    ...
    # tool_call_ids of engine-internal calls: every later frame for them
    # (update, result) is dropped too, so no orphan "🔧 tool" row is minted
    # for a result whose call was filtered.
    ...
    for params in updates:
        ...
        for ev in events:
            ...
            if tcid in internal_ids:
                continue
            if ev.kind == EVENT_TOOL_CALL:
                if ev.tool_name in ENGINE_INTERNAL_TOOLS or (
                    isinstance(raw_title, str) and raw_title.strip() in ENGINE_INTERNAL_TOOLS
                ):
                    internal_ids.add(tcid)
                    continue
                row = {
                    "role": "tool",
