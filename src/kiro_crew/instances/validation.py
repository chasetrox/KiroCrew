...
destination: it dials the fixed ``constants.LOOPBACK_HOST``. The one thing it
can be refused for is the config gate, which is what
...
class LoopbackValidationError(ValueError):
    """Raised when the loopback transport is refused: it is off in config.

    The transport has no destination field to validate — its address is the
    fixed ``constants.LOOPBACK_HOST`` — so the config gate
    (``instances.allow_loopback_transport``) is the only refusal it carries.
    """
