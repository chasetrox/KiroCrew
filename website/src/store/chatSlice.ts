  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(warmSlotCache.fulfilled, (state, action) => {
        ...
        // Read the cached corpus BEFORE the overwrite: the stitch below may only
        // keep rows cut from the same corpus as this warm's page.
        const cachedCorpus = state.slotTranscriptSource?.[safeKey(key)]?.source === 'acp_replay' ? 'acp_replay' : 'jsonl'
        const warmCorpus = action.payload.transcriptSource?.source === 'acp_replay' ? 'acp_replay' : 'jsonl'
        if (action.payload.transcriptSource) (state.slotTranscriptSource ??= {})[safeKey(key)] = action.payload.transcriptSource
        else if (state.slotTranscriptSource) delete state.slotTranscriptSource[safeKey(key)]
        ...
        // Rows cut from a DIFFERENT corpus than this page (the replay came up or
        // went away between the cache fill and this warm) cannot be stitched:
        // their mids and ts order say nothing about the other corpus, and the
        // provenance written above now names the page's corpus, so a head kept
        // from the old one would be read as this corpus by switchSlot's cursor
        // arithmetic. Same rule as switchSlot: no head across a corpus change.
        const cachedRows = cachedCorpus === warmCorpus ? (state.slotMessages[safeKey(key)] ?? []) : []
        const priorAll = hydrateQueuedBubbles(cachedRows, queue)
