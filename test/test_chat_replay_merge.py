...
def test_kas_steer_delivered_as_own_prompt_heads_its_own_turn() -> None:
    # kas (v3) delivers a steer as its OWN prompt, wrapped in <user_message>;
    # the JSONL steer row sits inside the previous prompt's group. The replayed
    # steer turn must claim that row (keeping its steer flag) and the steered
    # answer must render once, under it -- not as a grey wire-text user bubble
    # plus a sidecar copy of the answer under the first prompt.
    updates = [
        _prompt("go"),
        _text("agent_message_chunk", "going"),
        _text("user_message_chunk", "<user_message>also do X please</user_message>"),
        _text("agent_message_chunk", "doing X"),
    ]
    jsonl = [
        _row("user", "go", mid="u1"),
        _row("assistant", "going"),
        _row("user", "also do X please", steer=True, mid="u2"),
        _row("assistant", "doing X"),
    ]
    out, report = merge_replay_transcript(updates, jsonl)
    assert _sources(out) == [
        ("user", SOURCE_JSONL),
    ...
    assert out[2]["meta"]["steer"] is True and out[2]["meta"]["mid"] == "u2"
    assert [r["content"] for r in out if r["role"] == "assistant"] == ["going", "doing X"]
    assert report["turns"] == 2


def test_engine_internal_tool_after_last_answer_is_dropped() -> None:
    # kas/v3 replays its own fetch_cloud_config call AFTER the last answer too
    # (not only before the first prompt), where the pre-turn rule cannot catch
    # it; live never showed it, so the replay must not either -- call, update
    # and result alike, and no orphan "🔧 tool" row for the filtered result.
    updates = [
        _prompt("hi"),
        ...
        _tool_done("cfg", "{}"),
        _prompt("more"),
        _tool_call("cfg2", "fetch_cloud_config", {}, kind="other"),
        _tool_call("rd", "read", {"path": "a.txt"}),
        _tool_done("rd", "first line"),
        _tool_done("cfg2", "{}"),
        _text("agent_message_chunk", "done"),
    ]
    out, _ = merge_replay_transcript(updates, [_row("user", "hi"), _row("user", "more")])
    assert [r["role"] for r in out] == ["user", "assistant", "user", "tool", "assistant"]
    assert "fetch_cloud_config" not in json.dumps(out)
    assert out[3]["meta"]["tool_call_id"] == "rd"
